...
### v0.6.0 (当前) - 2025年2月
- ✅ **连线级条件与效果**：连线支持独立的前置条件和效果，实现更复杂的状态驱动叙事
- ✅ **状态历史追踪**：模拟器支持记录状态变化历史，可随时回退到任意历史状态
- ✅ **冲突检测系统**：自动检测不可达节点、剧情死胡同、矛盾的条件逻辑
- ✅ **状态评估工具**：统一的条件评估和效果应用工具类
- ✅ **连线编辑增强**：属性面板支持编辑连线的条件和效果
...
- [ ] 状态依赖可视化视图（按变量分组显示依赖关系）
- [ ] 画布/树视图的状态指示器（显示节点/连线条件徽章）
- [ ] 数据库视图的状态依赖列
- [ ] 选项条件评估（不满足的选项显示为禁用）
- [ ] Group Frame 的剧情池/剧情块功能
- [ ] 路径对比工具（并排显示不同选择路径的最终状态）
- [ ] 导出为脚本或游戏引擎格式
- [ ] 多语言支持（英文界面）
